        ...
        <div className="drs-container py-8 md:py-12">
          <div className="flex flex-col lg:flex-row gap-0 items-stretch min-h-[300px] lg:min-h-[400px]">
            <div className="relative w-full lg:w-1/2" style={{ position: 'relative', zIndex: 1 }}>
              <img src={imgPcs} alt="Patient Centric Services" className="w-full h-full min-h-[200px] lg:min-h-[400px] object-cover rounded-t-[24px] lg:rounded-t-none lg:rounded-l-[32px] lg:rounded-tl-[40px] lg:rounded-bl-[40px]" />
            </div>
            <div className="relative w-full lg:w-1/2 flex flex-col justify-center overflow-hidden rounded-b-[24px] lg:rounded-b-none lg:rounded-r-[32px] lg:rounded-tr-[40px] lg:rounded-br-[40px] p-[4rem_0.5rem_0.5rem_0.5rem] lg:p-0 mt-[-50px] lg:mt-0" style={{ backgroundColor: '#69C0AC' }}>
              <div className="lg:ml-[-35px] lg:w-[calc(100%+35px)] lg:p-[5.5rem]">
                <span className="text-[16px] lg:text-[20px] hero-tag-mobile" style={{ color: '#000', fontStyle: 'normal', fontWeight: 400, lineHeight: '40px', textTransform: 'uppercase', borderRadius: '30px', border: '1px solid #000', padding: '0px 20px', display: 'inline-block', marginBottom: '1rem', width: 'fit-content', background: 'transparent' }}>{t('pcs.hero.tag')}</span>
                <h1 className="text-[24px] md:text-[30px] lg:text-[35px] leading-[30px] md:leading-[36px] lg:leading-[40px] mb-4" style={{ color: '#000', fontWeight: 900 }}>{t('pcs.hero.title')}</h1>
                <p className="text-[16px] md:text-[18px] lg:text-[20px] leading-[22px] md:leading-[24px] lg:leading-[25px]" style={{ color: '#000', fontWeight: 400 }}>{t('pcs.hero.description')}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-12">
            <div className="col-span-12 lg:col-start-3 lg:col-span-7" style={{ marginBottom: '6.5rem' }}>
              <div className="flex flex-col gap-6 mb-12">
                <p style={{ color: '#000', fontSize: '20px', fontWeight: 400, lineHeight: '1.5' }}>{t('pcs.text1')}</p>
                <p style={{ color: '#000', fontSize: '20px', fontWeight: 400, lineHeight: '1.5' }}>{t('pcs.text2')}</p>
              </div>
              <img src={imgPcsSingle} alt="Soluções PCS" className="w-full h-auto" />
            </div>
          </div>
        </div>
      </section>
                ...
                {[1,2,3,4,5,6].map((i) => (
                  <div key={i} className="col-span-1" style={{ marginTop: i === 2 ? '65px' : i === 3 ? '130px' : i === 4 ? '-110px' : i === 5 ? '-45px' : i === 6 ? '20px' : '0' }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="auto" viewBox="0 0 422 379" fill="none">
                      ...
                      <text className="botao" x="23" y={i === 1 || i === 2 ? "185" : "150"} fontSize="20" fontWeight="400" fill="#008C79">{t(`pcs.svg${i}.desc1`)}</text>
                      <text className="botao" x="23" y={i === 1 || i === 2 ? "210" : "175"} fontSize="20" fontWeight="400" fill="#008C79">{t(`pcs.svg${i}.desc2`)}</text>
                      <text className="botao" x="23" y={i === 1 || i === 2 ? "235" : "200"} fontSize="20" fontWeight="400" fill="#008C79">{t(`pcs.svg${i}.desc3`)}</text>
        ...
        <div className="drs-container">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center" style={{ borderRadius: '50px', padding: '70px', backgroundColor: '#F39325' }}>
            <div className="lg:col-span-4">
              <h2 style={{ fontWeight: 900, fontSize: '30px', color: '#000', lineHeight: '1.2' }}>{t('pcs.cta.title')}</h2>
            </div>
            <div className="lg:col-span-5 flex items-center">
              <Link to="/contato" className="flex items-center gap-4" style={{ borderRadius: '10px', background: '#274B41', padding: '15px 25px' }}>
                ...
                <span style={{ fontSize: '16px', color: '#fff', fontWeight: 400 }}>{t('pcs.cta.button')}</span>
              </Link>
            </div>
          </div>
        </div>
